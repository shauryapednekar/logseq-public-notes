...
- $cis(\alpha + \beta) = cis(\alpha) \cdot cis(\beta)$
	- Geometrically, this makes sense because the left side is the vector on the unit circle that is formed by the sum of both the angles, whereas the right side