...
- Python output buffering
	- Python buffers its output. However, it uses line buffering only if both `stdin` and `stdout` are a console. When we employ file redirection (such as when using `docker logs` to view the output of a python script running inside a docker container) or pipes, Python performs full buffering. This means
-