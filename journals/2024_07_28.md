- [[retrieval augmented generation]]
- [[batch normalization layer]]
- [[homepage]]
- [[env-setup/pc]] Ran [these](https://stackoverflow.com/a/48957722) steps to run [[docker]] without `sudo` on the pc.
	- ```bash
	  sudo groupadd docker
	  sudo usermod -aG docker $USER
	  newgrp docker
	  ```
	- Essentially, this creates a new group called `docker` (which probably already exists since it's created during docker installation). It then adds the current user to that group, and refreshes the users
	- This works because communication with the docker unix socket is only allowed by the `root` user and the `docker` group.